      ...
      <div className="flex flex-col h-full animate-in fade-in duration-500">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="p-2 hover:bg-slate-100 rounded-lg border border-slate-200 transition-colors">
              <ChevronLeft className="w-5 h-5 text-slate-600" />
            </Button>
            <Button variant="outline" className="p-2 hover:bg-slate-100 rounded-lg border border-slate-200 transition-colors">
              <ChevronRight className="w-5 h-5 text-slate-600" />
            </Button>
            <Button variant="primary" className="ml-2 flex items-center gap-2 text-white px-4 py-2 rounded-lg transition-colors shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-grow overflow-hidden">
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="grid grid-cols-7 border-bottom border-slate-100 bg-slate-50">
            ...
            <div className="grid grid-cols-7 flex-grow divide-x divide-y divide-slate-100 border-t border-slate-100">
              {startPadding.map(i => <div key={`p-${i}`} className="bg-slate-50/50" />)}
              {days.map(day => {
                const event = CONSTANT.calendarEvents.find(e => e.day === day);
                return (
                  <div key={day} className={`min-h-[100px] p-2 hover:bg-slate-50 transition-colors group relative cursor-pointer ${day === 2 ? 'bg-orange-50/30' : ''}`}>
                    <span className={`text-sm font-semibold ${event ? 'text-slate-900' : 'text-slate-400'}`}>
                      {day}
                    </span>
                    {event && (
                      <div className={`mt-2 p-1.5 rounded-md ${event.color} text-white shadow-sm`}>
                        <div className="text-[10px] font-bold leading-tight truncate">{event.title}</div>
                        <div className="text-[9px] opacity-90 truncate">{event.location}</div>
                      </div>
                    )}
              ...
              <div className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Bay 4 temp at 42°F (Threshold: 38°F)</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Sector C requires manual valve check</p>
                  </div>
                </div>
              </div>
            </div>
