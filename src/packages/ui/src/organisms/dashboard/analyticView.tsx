...
type StatCardType = { 
  ...
  const StatCard = ({ title, value, change, trend, icon: Icon, color }: StatCardType) => (
      ...
      <div className="min-h-screen bg-slate-50/50 p-6 md:p-10 space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Monitoring harvest health and inventory flow.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={CONSTANT.categoryData} layout="vertical" barSize={20}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {[
              { item: 'Blood Oranges', issue: 'Expiring in 2 days', status: 'Priority', color: 'text-amber-600' },
              { item: 'Organic Bananas', issue: 'Overstocked', status: 'Stock Alert', color: 'text-blue-600' },
              { item: 'Strawberries', issue: 'Quality Variance Detected', status: 'Investigate', color: 'text-rose-600' }
            ].map((alert, i) => (
              <div key={i} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between">
                ...
                <span className={`text-xs font-bold px-3 py-1 rounded-full bg-slate-100 ${alert.color}`}>
                  {alert.status}
                </span>
              </div>
            ))}
