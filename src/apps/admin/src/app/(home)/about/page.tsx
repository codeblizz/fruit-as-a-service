...
const StatCard = ({ label, value }: any) => (
  <div className="text-center p-6 bg-white/50 backdrop-blur-sm rounded-2xl border border-stone-100 shadow-sm">
    <div className="text-3xl font-bold text-emerald-800 mb-1">{value}</div>
    <div className="text-stone-500 text-sm font-medium uppercase tracking-wider">{label}</div>
  </div>
);

const ValueCard = ({ icon: Icon, title, description }: any) => (
  <div className="flex flex-col items-start p-8 rounded-3xl bg-white border border-stone-100 hover:shadow-xl hover:shadow-emerald-900/5 transition-all duration-300 group">
    ...
    <div className="min-h-screen bg-stone-50 selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <header className="pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-serif font-medium text-stone-900 tracking-tight leading-[1.1] mb-8">
            Reimagining the <span className="italic text-emerald-700">fruit chain</span> <br /> from soil to software.
          </h1>
          <p className="max-w-2xl mx-auto text-stone-600 text-lg md:text-xl leading-relaxed font-light">
            We're a team of agronomists, engineers, and fruit lovers building the digital infrastructure 
            for a more transparent and delicious food system.
          </p>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="space-y-8">
            ...
            <p className="text-stone-600 leading-relaxed">
              For too long, the fruit industry has prioritized shelf-life over flavor. Large-scale logistics require fruit to be picked long before it's ready, resulting in bland produce and immense carbon footprints.
            </p>
            <div className="space-y-6">
              {[
                { title: "Direct-from-Orchard", desc: "We bypass central warehouses, moving fruit from the tree to your door in under 36 hours." },
                { title: "AI-Driven Ripeness", desc: "Our vision systems analyze fruit condition to ensure you only get items at their peak sugar levels." }
              ].map((item, idx) => (
      ...
      <section className="py-24 px-6 bg-stone-50">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-serif text-stone-900 mb-4">Our Core Values</h2>
            <p className="text-stone-500">The principles that guide every line of code we write.</p>
          </div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-4xl mx-auto text-center bg-emerald-900 rounded-[3rem] p-12 md:p-20 text-white shadow-2xl shadow-emerald-950/20">
          ...
          <h2 className="text-4xl font-serif mb-6">Join the Green Revolution</h2>
          <p className="text-emerald-100/80 mb-10 text-lg">
            We're always looking for growers, developers, and designers who want to build a better future for agriculture.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button className="bg-white text-emerald-900 px-8 py-4 rounded-full font-bold hover:bg-emerald-50 transition-all">View Openings</button>
            <button className="bg-emerald-800 text-white border border-emerald-700 px-8 py-4 rounded-full font-bold hover:bg-emerald-700 transition-all">Partner with Us</button>
          </div>
        </div>
      </section>
    </div>
