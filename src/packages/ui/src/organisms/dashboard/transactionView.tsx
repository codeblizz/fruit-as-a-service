...
const TransactionView = () => {
  ...
  const filteredTransactions = useMemo(() => {
    return CONSTANT.transactionData.filter(trx => {
      const matchesSearch = trx.entity.toLowerCase().includes(searchTerm.toLowerCase()) || 
    ...
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Monitor and manage all your incoming and outgoing payments.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-xl bg-white text-slate-700 hover:bg-slate-50 transition-colors text-sm font-medium">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              {filteredTransactions.map((trx) => (
                <tr key={trx.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${trx.amount > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                        {trx.amount > 0 ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-900">{trx.entity}</p>
                        <p className="text-xs text-slate-400 font-medium">{trx.id} • {trx.method}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-sm font-bold ${trx.amount > 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
                      {trx.amount > 0 ? '+' : ''}{trx.amount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-bold ${getStatusStyle(trx.status)}`}>
                      {getStatusIcon(trx.status)}
                      {trx.status}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button className="p-2 text-slate-400 hover:text-slate-900 hover:bg-white rounded-lg transition-all border border-transparent hover:border-slate-200 shadow-none hover:shadow-sm">
        ...
        <div className="p-4 md:p-6 border-t border-slate-100 flex items-center justify-between">
          <p className="text-xs font-bold text-slate-400">
            Showing {filteredTransactions.length} of {CONSTANT.transactionData.length} results
          </p>
          <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-1 bg-slate-900 text-white rounded-lg text-xs font-bold">1</button>
            <button className="px-3 py-1 hover:bg-slate-50 rounded-lg text-xs font-bold text-slate-600">2</button>
