...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        setLoader(true);
        const response = await fetch(`/api/fruits/orders?email=${session.user.email}`);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-8">
      <Card name='' className="w-full max-w-6xl p-8">
        ...
        <p className="text-gray-600 mb-8">View your past orders and subscriptions</p>
                ...
                <div className="border-t pt-4 flex justify-between items-center">
                  <div>
                    <span className="text-gray-600">
                      {order.type === 'subscription' ? `Subscription - ${order.subscriptionPlan}` : 'One-time purchase'}
                    </span>
                  </div>
