...
const DashboardOverview = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-6 md:p-8 animate-in fade-in slide-in-from-bottom-2 duration-500 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-extrabold text-apple-green uppercase tracking-widest">Analytics Dashboard</span>
          </div>
          <h1 className="text-3xl font-extrabold text-blackcurrant tracking-tight">
          Dashboard Overview
          </h1>
          <p className="text-slate-500 text-sm font-medium">
            Welcome back, Greenhouse Lead. Here's your harvest data for today.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <NextLink
            ...
            className="group flex items-center gap-2 bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 transition-all active:scale-95"
          >
            <Plus size={18} className="text-slate-400 group-hover:text-emerald-500 transition-colors" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {CONSTANT.dashboardStats.map((stat, i) => (
          <div
            key={i}
            className="relative group bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all overflow-hidden"
          >
            ...
            <div className="mt-6 -mx-2 opacity-50 group-hover:opacity-100 transition-opacity">
              <svg viewBox="0 0 100 20" className="w-full h-12 overflow-visible">
                <path
                  d={`M 0 20 ${stat.trend.map((h, idx) => `L ${(idx * 100) / 6} ${20 - (h / 5)}`).join(' ')}`}
                  fill="none"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500">Monthly harvest performance metrics</p>
            </div>
            <button className="p-2 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-100 hover:text-slate-600 transition-all">
              <Filter size={18} />
            </button>
          </div>

          <div className="h-64 flex flex-col items-center justify-center text-center bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
             <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center mb-4 shadow-sm">
              ...
              <h4 className="text-base font-bold text-slate-800 tracking-tight">Gathering Harvest Data...</h4>
              <p className="text-slate-500 max-w-xs mt-1 text-xs font-medium">
                Detailed visualizations will be available as soon as your sensors sync with the orchard gateway.
              </p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-apple-green via-primary to-kiwi rounded-[2.5rem] p-8 text-white shadow-sm shadow-apple-green relative overflow-hidden group">
          ...
          <div className="relative z-10 h-full flex flex-col">
            ...
            <h3 className="text-2xl font-black mb-3">Optimize your yield today.</h3>
            <p className="text-emerald-50 text-sm leading-relaxed mb-8 opacity-90">
              Based on recent UV indexes, we recommend adjusting the irrigation cycle in Segment B by 15 minutes.
            </p>
      ...
      <div className="flex items-center justify-center gap-4 py-4 text-slate-400">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.2em]">System Version 4.2.0 • Real-time Monitoring Active</p>
        <div className="h-px flex-1 bg-slate-200"></div>
      </div>
    </div>
  );
